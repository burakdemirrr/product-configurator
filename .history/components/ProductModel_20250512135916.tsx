...
export function ProductModel({ modelPath, onError }: ProductModelProps) {
  ...
  let gltf;
  try {
    gltf = useGLTF(modelPath, undefined, onErrorHandler);
  } catch (error) {
  ...
  useEffect(() => {
    if (!modelRef.current || modelError || !gltf?.scene) return;
    ...
    try {
      modelRef.current.add(gltf.scene.clone());
    } catch (error) {
