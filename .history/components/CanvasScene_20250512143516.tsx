...
// Preload the model
useGLTF.preload('/models/car_glb.glb', true);
...
export function CanvasScene() {
  ...
  useEffect(() => {
    console.log('Attempting to load model from: /models/car_glb.glb');
    
    // Check if model exists
    fetch('/models/car_glb.glb')
      .then(response => {
        console.log('Model fetch response:', response.status, response.ok);
        if (!response.ok) {
          console.error('Model file not found or inaccessible');
          setModelError(true);
        }
      })
      .catch(error => {
